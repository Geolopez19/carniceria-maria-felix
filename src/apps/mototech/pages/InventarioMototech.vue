  ...
  <div class="p-2 sm:p-4 md:p-6">
    ...
    <Tabs value="0">
      ...
      <TabPanels>
        ...
        <TabPanel value="0">
          <!-- Métricas para MotoTech -->
          <div class="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4 mb-6 mt-4">
            ...
            <Card class="bg-purple-50 border-none shadow-sm">
              ...
              <template #content><span class="text-xl md:text-2xl font-bold text-purple-900">C${{ Number(metricas.inversionTotal || 0).toLocaleString('es-NI', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span></template>
            </Card>
            <Card class="bg-emerald-50 border-none shadow-sm">
              <template #title><span class="text-xs md:text-sm font-medium text-emerald-700 uppercase">Valor Venta</span></template>
              <template #content><span class="text-xl md:text-2xl font-bold text-emerald-900">C${{ Number(metricas.valorTotal || 0).toLocaleString('es-NI', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span></template>
            </Card>
            <Card class="bg-slate-50 border-none shadow-sm">
              <template #title><span class="text-xs md:text-sm font-medium text-slate-700 uppercase">Ganancia Proyectada</span></template>
              <template #content><span class="text-xl md:text-2xl font-bold text-slate-900">C${{ Number(metricas.gananciaProyectada || 0).toLocaleString('es-NI', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span></template>
            </Card>
          </div>
          ...
          <div class="flex flex-col md:flex-row gap-4 mb-4 bg-white p-4 rounded-lg shadow-sm border border-gray-100">
            <IconField iconPosition="left" class="flex-1 w-full">
              ...
              <InputText v-model="busqueda" placeholder="Buscar producto por nombre, código, marca o modelo..." class="w-full" />
            </IconField>
          ...
          <div class="card shadow-md rounded-lg overflow-hidden border border-gray-200 bg-white">
            ...
            >
              ...
              <Column field="nombre" header="Nombre" sortable>
                <template #body="{ data }">
                  <div class="flex flex-col">
                    <span class="font-bold text-slate-800">{{ data.nombre }}</span>
                    <span v-if="data.modelo" class="text-[11px] text-slate-400 font-medium">Mod: {{ data.modelo }}</span>
                  </div>
                </template>
              </Column>
              ...
              <Column field="marca" header="Marca" sortable>
                <template #body="{ data }">
                  <span class="font-bold text-amber-800 bg-amber-50 px-2 py-0.5 rounded text-xs border border-amber-200">{{ data.marca || '—' }}</span>
                </template>
              </Column>
              <Column field="talla" header="Talla" sortable class="text-center">
                <template #body="{ data }">
                  <Tag v-if="data.talla" :value="data.talla" severity="info" class="font-bold text-xs" />
                  <span v-else class="text-slate-400">—</span>
                </template>
              </Column>
              <Column field="color" header="Color" sortable class="hidden md:table-cell">
                <template #body="{ data }">
                  <span class="text-xs text-slate-600">{{ data.color || '—' }}</span>
                </template>
              </Column>

              <Column field="categoria" header="Categoría" sortable class="hidden md:table-cell">
                <template #body="{ data }">
                  <span class="text-xs text-slate-600 bg-slate-50 px-2 py-0.5 rounded">{{ data.categoria || 'General' }}</span>
                </template>
              </Column>
              <Column field="stock" header="Stock" sortable>
                <template #body="{ data }">
                  <Badge :value="`${data.stock || 0} ${data.unidad_medida || 'und'}`" :severity="data.stock < 3 ? 'danger' : data.stock > 10 ? 'success' : 'warn'" />
                </template>
              </Column>
              
              <!-- Precio Compra (Costo) -->
              <Column field="costo" header="P. Compra (Costo)" sortable>
                <template #body="{ data }">
              ...
              <!-- Precio Venta -->
              <Column field="precio" header="P. Venta" sortable>
                <template #body="{ data }">
                  <span class="font-bold text-emerald-700">{{ formatCurrency(data.precio || 0) }}</span>
                </template>
              </Column>

              <!-- Margen / Ganancia -->
              <Column header="Margen / Ganancia" sortable class="hidden lg:table-cell">
                <template #body="{ data }">
                  <div class="flex items-center gap-1">
                    <span 
                      class="font-mono text-xs font-bold px-1.5 py-0.5 rounded"
                      :class="((data.precio || 0) - (data.costo || 0)) >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'"
                    >
                      +{{ formatCurrency((data.precio || 0) - (data.costo || 0)) }}
                    </span>
                  </div>
                </template>
              </Column>
    ...
    <Dialog v-model:visible="modalEtiqueta.visible" header="Etiqueta del Producto" modal class="w-full max-w-[90vw] md:max-w-md">
      <div class="flex flex-col items-center gap-4 py-4" v-if="modalEtiqueta.producto" id="etiqueta-imprimible">
        <div class="text-center">
          ...
          <p class="text-xs text-slate-500 font-medium mt-0.5">{{ companyStore.currentCompany.name }}</p>
          <div class="text-lg font-bold text-indigo-600 mt-1">
            {{ formatCurrency(modalEtiqueta.producto.precio) }} / {{ modalEtiqueta.producto.unidad_medida || (companyStore.isMotoTech ? 'und' : 'lb') }}
          </div>
        </div>
    ...
    <html>
      ...
      <body>
        <div class="label-container">
          <div style="width:100%">
            <div class="business-name">${companyStore.currentCompany.name.toUpperCase()}</div>
            <h2 class="title">${prod.nombre}</h2>
            <div class="precio">PRECIO: C$${Number(prod.precio || 0).toFixed(2)}</div>
          </div>
...
const cargarMetricasYCategorias = async () => {
  try {
    ...
    metricas.value.stockTotal = totalStock
    metricas.value.valorTotal = valorVenta
    metricas.value.inversionTotal = inversion
    metricas.value.gananciaProyectada = valorVenta - inversion
    metricas.value.bajoStock = todos.filter(p => (p.stock || 0) < 3).length
